...
const { TabPane } = Tabs;
const fakeDataUrl = 'https://randomuser.me/api/?results=10&inc=name,gender,email,picture&noinfo';
              ...
              renderItem={item => (
                <List.Item actions={[<Icon type="close-circle-o" />, <Icon type="right-circle-o" />]}>
                  <List.Item.Meta
                    ...
                    description={`Hey my email is ${item.email}`}
                  />
                </List.Item>
              )}
            />
          </TabPane>
          <TabPane tab="Notifications" key="2">Hi mum</TabPane>
        </Tabs>
      </div>
    );
  }
